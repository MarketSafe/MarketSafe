...
function statusFiliais(req, res) {
...
function alertasMensal(req, res) {
  const emailAutenticacao = req.body.emailAutenticacao;
  const senhaAutenticacao = req.body.senhaAutenticacao;
      ...
      .then(function (resultadoAutenticar) {
        ...
        if (resultadoAutenticar.length === 1) {
          ...
          if (funcionarioAutenticado.cargo === "gerente") {
            ...
            const limite_linhas = 3;
            // console.log("Controllerranking: ")
            const { filial, mes } = req.body;

            if (filial, mes) {
              rankingModel.alertasMensal(filial, mes) // Chama o método de busca por filial
                .then(resultado => {
...
// function cardQtdAlerta(req, res) {
      ...
      .then(function (resultadoAutenticar) {
        ...
        if (resultadoAutenticar.length === 1) {
          ...
          if (funcionarioAutenticado.cargo === "gerente") {
            ...
            if (filial, mes) {
              rankingModel.cardQtdAlerta(filial, mes) // Chama o método de busca por filial
                .then(resultado => {
                  console.log(resultado)
                  if (resultado.length > 0) {
...
function statusFiliaisHistorico(req, res) {
  console.log(req.body)
  // declara as variáveis recebidas:
  if (req.body.funcionarioAutenticado.cargo === "gerente") {
    ...
    const { mes } = req.body;
      ...
      rankingModel.statusFiliaisHistorico(mes, req.body.funcionarioAutenticado.fk_empresa) // Chama o método de busca por filial
        .then(resultado => {
          ...
          if (resultado.length > 0) {
            ...
            res.status(204).send("Nenhum resultado encontrado para os dados!");
          }
        })
        .catch(erro => {
