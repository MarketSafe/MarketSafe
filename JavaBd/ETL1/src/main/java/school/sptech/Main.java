...
public class Main implements RequestHandler<S3Event, String> {
    ...
    private static final String DESTINATION_BUCKET = "s3-trusted-mktsf";
    ...
    public String handleRequest(S3Event s3Event, Context context) {
        ...
        if (sourceKey.contains("pix")) {
            ...
            try {
                ...
                List<Pix> stocks = mapper.map(s3InputStream);
            ...
            try {
                ...
                // Conversão do JSON para uma lista de objetos Stock usando o Mapper
                MapperDados mapper = new MapperDados();
                List<Dado> dados = mapper.map(s3InputStream);

                // Geração do arquivo CSV a partir da lista de Stock usando o CsvWriter
                CsvWriterDados csvWriter = new CsvWriterDados();
